...
// MAGIC This is **part 1** of 2 notebooks that demonstrate bulk load from blob storage into Azure Cosmos DB Cassandra API - of 1.5 GB of the Chicago crimes public dataset (6.7 million records).<BR>
...
// MAGIC 2. Upload to DBFS to the raw data persistence zone as parquet, create a Hive table on the raw dataset, optimize it for querying
// MAGIC 3. Format the raw dataset, persist to the curated data zone as parquet, create a Hive table on the curated dataset
// MAGIC <br>We will use this dataset to load to Azure Cosmos DB Cassandra API.<br>  The curated dataset will be used in the stream processing samples as well.
// MAGIC 
