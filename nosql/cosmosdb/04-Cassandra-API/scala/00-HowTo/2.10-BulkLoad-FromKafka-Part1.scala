...
// MAGIC   
// MAGIC With the three notebooks, we cover (publishing to Kafka and then), reading from Kafka in batch mode and sinking to an OLTP store - the Azure Cosmos DB Cassandra table, and an analytics store - the Databricks Delta table.<BR><BR>
// MAGIC 
// MAGIC This notebook also covers **provisioning and configuring HDInsight Kafka** for the exercise, also **provisioning Azure Cosmos DB Cassandra API**.
...
// MAGIC %md
// MAGIC #### 1.0.b. Specific to HDInsight Kafka - to be run on Azure portal
// MAGIC 
...
// MAGIC %md
// MAGIC #### 1.0.c. Specific to HDInsight Kafka - to be run on Ambari, on the Kafka cluster
// MAGIC 
...
// MAGIC https://docs.microsoft.com/en-us/azure/hdinsight/kafka/apache-kafka-connect-vpn-gateway#configure-kafka-for-ip-advertising<BR><BR>
// MAGIC   
...
// MAGIC %md
// MAGIC #### 1.0.d. Specific to Kafka and Spark - to be run on the Azure portal/Databricks cluster
