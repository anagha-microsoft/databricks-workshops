...
// MAGIC %md
// MAGIC ## 1.0. Basic stream consuming
...
// MAGIC %md
// MAGIC **Remember to:** Cancel the command above incase you have executed it.  We will need our cluster resources availble for subsequent steps.
...
// MAGIC %md
// MAGIC **Remember to:** Cancel the command above.  We will need our cluster resources availble for subsequent steps.
...
// MAGIC %md
// MAGIC Joining streams with static data is no different from joining two datafraes with static data.<br>
// MAGIC We will join our stream with the taxi zone dataset.
